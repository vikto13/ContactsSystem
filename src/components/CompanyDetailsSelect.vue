  ...
  <div class="md-layout-item m-3">
    ...
    >
      <input-box-icon
        ...
        :is-not-valid="
          isInvalid({ [select.id]: employee[select.id] }) && select.required
        "
        class="mt-2"
      >
        <select
          ...
          class="form-select"
          @input="
            $store.commit('setEmployee', { [select.id]: $event.target.value })
          "
        >
          <option selected :value="''" :disabled="employee[select.id] == ''">
