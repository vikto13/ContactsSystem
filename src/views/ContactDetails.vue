        ...
        <div v-for="(box, index) in info" :key="index" class="md-layout-item">
          ...
          <md-card-content>
            <p v-for="(column, position) in box.details" :key="position">
              {{ `${column.field} ${isNoText(column.getText(employee))}` }}
            </p>
          </md-card-content>
        </div>
      </div>
      ...
      info: [
        {
          ...
          details: [
            { getText: (text) => text.phone_number, field: "Telefono nr:" },
            { getText: (text) => text.email, field: "El paštas:" },
            { getText: (text) => text.office_id.name, field: "Adresas:" },
          ],
        },
        {
          title: "Kompanijos detalės:",
          details: [
            { getText: (text) => text.company_id.name, field: "Kompanija:" },
            { getText: (text) => text.division_id.name, field: "Būstinė:" },
            {
              getText: (text) => text.department_id.name,
              field: "Departamentas:",
            },
            { getText: (text) => text.group_id.name, field: "Grupė:" },
          ],
        },
      ],
    };
